...
class SudokuDiffusion(nn.Module):
    ...
    def forward(self, x, use_holding_gate=False):
        ...
        hold = self.holding(h).squeeze(-1)

        if use_holding_gate:
            corrupt_mask = (torch.sigmoid(hold) > 0.5).unsqueeze(-1)
            mask_tokens = torch.full_like(x, MASK_TOKEN)
            mask_emb = self.embedding(mask_tokens) + self.pos_embedding(positions)
            gated = torch.where(corrupt_mask, mask_emb, emb)
            h2 = self.transformer(gated)
            jump = self.jump_chain(h2)
        else:
            jump = self.jump_chain(h)
...
for epoch in range(num_epochs):
    ...
    for batch_puzzles, batch_solutions in loader:
        ...
        total_jump += jump_loss.item()
        total_hold += holding_loss.item()

    elapsed = time.time() - start
    print(f"Epoch {epoch+1}/{num_epochs} — "
          f"Loss: {total_loss/len(loader):.4f} — "
          f"Jump: {total_jump/len(loader):.4f} — "
...
#             jump, hold = model(x, use_holding_gate=True)
